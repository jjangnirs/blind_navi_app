...
| SR-F-089-H | 보행 녹색 신호 쿨다운 차단 해제 및 도로 하단/차량 신호등 분리 안정화 (ADR-023): 음성 중재기 카테고리 분리(`signal_decision_red` vs `signal_decision_green`)로 적색 직후 녹색 발화 차단 제거, 적색 멘트 중 녹색 신호 즉시 선점(`PREEMPT_AND_PLAY`), 녹색 안내 100% 보장 래치(`hasSpokenCurrentGreenPhase`), 물리적 등두 수직 한계 검증 및 도로 하단 정차 차량 브레이크등/후미등 오결합 배제(`isLowerRoadwayRed`), 가로형 차량 신호등 수용 및 트래커 단일 인스턴스화를 적용해야 한다. | P0 | 단위/알고리즘/기기 테스트 |
| SR-F-089-I | 숫자형 잔여시간 표시기(초록색 숫자) 클러스터링 및 2D 공간 추적 락(Spatial Tracking Lock-on) (ADR-024): 2D 유클리드 공간 거리 및 수직 가중치(1.4배) 기반 신호 추적 락을 적용하여 한손 파지 시 조준 박스의 상/하단 텔레포트 요동을 차단하고 Bounding Box EMA($\alpha=0.70$) 평활화를 적용해야 한다. 또한 디지털 잔여시간 표시기(7-segment, 도트 매트릭스 LED)의 십의 자리/일의 자리 분절 획을 `clusterDigitBlobs`로 병합하고 종횡비 필터를 $W \le 1.65 H$로 정밀화하여 녹색 숫자 카운트다운을 100% 연속 인식해야 한다. 가공 차량 신호기 판정 고도를 $Y < 0.12f$로 최적화하고 2D 거리 비교로 배경 원거리 적색등에 의한 Zero False-Green 오판정을 차단해야 한다. | P0 | 단위/알고리즘/기기 테스트 |
| SR-F-089-J | 도심 협곡 GPS 수신율 보정 및 지도 흔들림(Pan/Rotation 충돌 및 4.5° 불감대) 안정화 (ADR-025): 도심 고층 빌딩 다중경로 반사(Multipath)로 인한 오차 반경(35m~50m) 발생 시 GPS 신호 강도 계산을 지수 평활화 곡선으로 보정하여 50~55%(보통) 수준을 안정 표출해야 한다. 또한 90ms 헤딩 센서 갱신 시 Leaflet 지도 패닝 애니메이션이 초당 11회 연속 충돌하는 현상을 방지하기 위해 사용자 이동 거리 1.5m 디바운싱을 적용하고, 지도 회전 불감대를 4.5도로 상향하여 보행 시 손/팔 진자 흔들림($\pm 4.5^\circ$)을 완전히 흡수하며, 보행 속도 임계값을 0.65 m/s로 완화해 천천히 걷더라도 지도가 도로 진행 방향 위(Heading-Up)로 견고히 고정되어야 한다. | P0 | 단위/기기/WebView 테스트 |
