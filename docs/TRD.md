...
### ADR-025 — 도심 협곡 GPS 수신율 보정 및 지도 흔들림(Pan/Rotation 충돌 및 4.5° 불감대) 안정화

- 결정:
  1. `LocationSample`에서 도심 고층 빌딩 다중경로 반사(Multipath)로 인한 오차 반경(35m~50m) 발생 시 기본 점수가 25%로 급락하던 계단식 계산식을 지수 평활화 곡선으로 보정하여, 정상 다중 위성 수신 중일 때 50~55%(보통) 수준을 안정적으로 유지하도록 개선한다.
  2. `RealRouteMapView`에서 90ms 헤딩 센서 갱신 시 `map.panTo` 애니메이션이 초당 11회 연속 취소/재시작되던 인터럽트 충돌을 방지하기 위해, 사용자 좌표가 이전 패닝 위치 대비 1.5m 이상 실질 이동했을 때만 지도 패닝을 실행하도록 디바운싱을 적용한다.
  3. `RealRouteMapView`의 지도 회전 불감대(Deadband)를 2.5도에서 4.5도로 상향하여, 정상 보행 시 발생하는 인체 팔 흔들림(Gait Sway, $\pm 4.5^\circ$)을 완전히 흡수하고 지도가 도로 진행 방향으로 흔들림 없이 고정되도록 구현한다.
  4. `NavigationViewModel`에서 보행 중 GPS 코스 상보 필터 결합 속도 임계값을 0.8 m/s에서 0.65 m/s(약 2.3 km/h, 완만한 보행)로 낮추어 천천히 걷더라도 지도가 도로 방향으로 묵직하게 고정되도록 개선한다.
