...
### 26. 도심 협곡 GPS 수신율 보정 및 지도 흔들림(Pan/Rotation 충돌 및 4.5° 불감대) 안정화 (`RealRouteMapView`, `LocationSample`, `NavigationViewModel`)
- **도심 GPS 수신율 지수 곡선 평활화 (`LocationSample`)**:
  - 고층 빌딩 다중경로 반사(Multipath)로 인한 오차 반경(35m~50m) 발생 시 급락하던 계단식 계산식을 보정하여 정상 위성 수신 중 50~55%(보통) 수준을 안정 표출 (야외 35% 하락 오인 해소)
- **지도 위치 패닝(Pan) 디바운싱 (`RealRouteMapView`)**:
  - 사용자 좌표가 이전 패닝 위치 대비 1.5m 이상 실질 전진했을 때만 `map.panTo` 애니메이션을 실행하여, 90ms 센서 갱신에 의한 초당 11회 패닝 취소/재시작 및 화면 미세 떨림 완전 제거
- **지도 회전 불감대(Deadband) 4.5° 상향 (`RealRouteMapView`)**:
  - 보행 시 발걸음 좌우 진자 흔들림($\pm 4.5^\circ$)을 무시하고 지도가 도로 진행 방향으로 흔들림 없이 고정되도록 개선
